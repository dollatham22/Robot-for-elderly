...
if (pulseSensor.sawStartOfBeat()) {            // Constantly test to see if "a beat happened". 
 ...
 if (trigger3==true){
    trigger3count++;
    //Serial.println("trigger3count:"+trigger3count);
    if (trigger3count>=10){ 
       angleChange = pow(pow(gx,2)+pow(gy,2)+pow(gz,2),0.5);
       //delay(10);
       Serial.println(angleChange); 
       if ((angleChange>=0) && (angleChange<=3)){ //if orientation changes remains between 0-10 degrees
...
//   BTSerial.write('B');
 ...
 if (AM<=7 && trigger2==false || AM>=15 && trigger2==false ){ //if AM breaks lower threshold (0.4g)
   trigger1=true;
   BTSerial.write('A');
